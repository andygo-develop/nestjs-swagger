...
### Added
- `SwaggerModule.forRoot({ title })` — set the document title without touching the environment.

### Changed
- Versioned documents now list unversioned routes alongside versioned ones.

### Fixed
- Percent-encoded credentials in `SWAGGER_URI` are decoded before basic auth compares them.
```
